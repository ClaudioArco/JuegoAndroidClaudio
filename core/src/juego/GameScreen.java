...
public class GameScreen extends ScreenAdapter {
	...
	private Array<Rectangle> arrayLluvia;
	private Array<Rectangle> arrayAcid;
	...
	public void show() {
		...
		lluviaImage = new Texture(Gdx.files.internal("droplet.png"));
		...
		arrayLluvia = new Array<Rectangle>();
		arrayAcid = new Array<Rectangle>();
		spawnRaindrop();
	}
	...
	public void update() {
		...
		for (Iterator<Rectangle> iterRain = arrayLluvia.iterator(); iterRain.hasNext(); ) {
		...
		for (Iterator<Rectangle> iterAcid = arrayAcid.iterator(); iterAcid.hasNext(); ) {
	...
	public void draw() {
		...
		for (Rectangle raindrop : arrayLluvia) {
			game.batch.draw(lluviaImage, raindrop.x, raindrop.y);
		}

		for (Rectangle aciddrop : arrayAcid) {
	...
	private void spawnRaindrop() {
		...
		if (chanceAcid < 40) {
			arrayAcid.add(raindrop);
		} else {
			arrayLluvia.add(raindrop);
		}
